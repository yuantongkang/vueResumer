<template>
  <div id="resumePreview">
    ...
    <section data-name="projects" v-show="resume.education">
       <h2>项目经历</h2>
       <ol>
         <li v-for="item in resume.projects" :key="item">
           <h3>{{item.name}}</h3>
    ...
    <section data-name="workHistory" v-show="resume.workHistory">
      <h2>工作经历</h2>
      <ol>
        <li v-for="item in resume.workHistory" :key="item">
          <h3>{{item.company}}</h3>
          <p v-show="item.content"> {{item.content}} </p>
        </li>
      </ol>
    ...
    <section data-name="awards" v-show="resume.awards">
       <h2>获奖情况</h2>
       <ol>
         <li v-for="item in resume.awards" :key="item">
           <h3>{{item.name}}</h3>
           <p v-show="item.content"> {{item.content}} </p>
         </li>
       </ol>
     ...
     <section data-name="contacts" v-show="resume.contacts">
       <h2>联系方式</h2>
       <table>
         <tr v-for="item in resume.contacts" :key="item">
           <td>{{item.contact}}</td>
